...
def midi_2_note_number(midi, length_factor, gradient_fraction = 3):
    ...
    start_gradients = np.ones((4,int(midi.shape[0]*length_factor)))
    end_gradients = np.ones((4,int(midi.shape[0]*length_factor)))
    last_print = 0
            ...
            if note[0,channel]>0: ## pitch
                try:
                    if note[1,channel] > 0: ## every note start
                        ...
                        general_gradient_amt = int(len(wave)/gradient_fraction)
                        general_gradient = []
                        ...
                        for j in range(0,len(wave)):
                            ...
                            try:
                                ...
                                end_gradients[channel,current_tick+len(wave)-j]=general_gradient[j]
...
def duration_2_wave(duration, start_gradient, end_gradient, gradient_fraction = 3, return_different_gradients = True):
    midi_wave = []
    last = 0
...
output = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/synced/waveforms only end gradient"
...
for set_num in range(start_index, len(sets)):
    ...
    while True:
        ...
        actual_start_gradients = orig_start_gradients[:,midi_tick:]
        actual_end_gradients = orig_end_gradients[:,midi_tick:]
        print("1")
        ...
        while good != "y":
            ...
            print(real_length)
            start_gradients = skimage.transform.rescale(actual_start_gradients,(1,float(good)))
            end_gradients = skimage.transform.rescale(actual_end_gradients,(1,float(good)))
            midi_duration = rescale_duration_graph(actual_midi_duration, float(good))
            print(len(midi_duration))
            midi,midi_start,midi_end,midi_start_end  = duration_2_wave(midi_duration, start_gradients, end_gradients)
        ...
        if not backing:
            ...
            save_array(midi_start_end, output+"/midis/start_end gradient/"+str(save_index)+".h5")
            save_index+=1

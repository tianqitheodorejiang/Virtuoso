...
def note_number_to_chroma(note_number):
    rescaled_note_number = np.round(skimage.transform.rescale(note_number, (1, rescale_factor, 1)))
           
sampling_size = 44100
path ="C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/all data wo trills"
base_output = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/synced/autosyncing pls bruv/"
...
for set_num in range(start_index, len(sets)):
    ...
    while True:
        ...
        if midi_wave.shape[0] < sampling_size*4:
            padding_amt = sampling_size*4-midi_wave.shape[0]
        ...
        wav_chroma_dos = librosa.feature.chroma_stft(y=orig_wav[wav_tick+sampling_size:wav_tick+sampling_size*2], sr=22050, tuning=0, norm=2,
                                                 hop_length=hop_size, n_fft=n_fft)
        
        midi_chroma = librosa.feature.chroma_stft(y=midi_wave, sr=22050, tuning=0, norm=2,
                                                 hop_length=hop_size, n_fft=n_fft)
        print("BLUB\n\n")
        print(current_array.shape,midi_chroma.shape,length_factor)
        print(wav.shape,wav_chroma.shape)
        print("BLUB\n\n")

...
output = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/synced/loaded arrays final only good"
...
for i in range(start,len(sets)):
    ...
    midi_wave, midi_graph, start_gradient_graph, end_gradient_graph = note_number_to_wave(complete_midi_graph, end_gradient = True, start_gradient = True, rescale_factor = 1)
    added = midi_wave[:complete_wav.shape[0]]+complete_wav[:midi_wave.shape[0]]
    added/=np.max(added)
    #sd.play(added,22050)
    if True:#input("gucc?:")!="n":
        save_array(midi_wave, output+"/"+set_+"/midi.h5")
        save_array(complete_wav, output+"/"+set_+"/wav.h5")

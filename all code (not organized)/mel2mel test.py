...
def mel2mel():
    inputs = keras.layers.Input((hp.max_T,hp.n_mels))
    print(inputs.shape)
    
    Y_logits, Y = AudioDec(inputs, training=True) # (B, T/r, n_mels)
    print(Y.shape)

    return keras.models.Model(inputs, [Y,Y_logits])


def loss(y_true,y_pred):
    # mel L1 loss
    loss_mels = tf.reduce_mean(tf.abs(y_pred[0] -y_true[0]))

    # mel binary divergence loss
    loss_bd1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=y_pred[1], labels=y_true[0]))
...
model = mel2mel()
pred = model.predict(np.zeros((1,hp.max_T,hp.n_mels)))[1][0]
print(pred.shape)

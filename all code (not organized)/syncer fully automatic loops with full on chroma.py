...
for set_num in range(start_index, len(sets)):
    ...
    midi_wave, midi_graph, start_gradient_graph, end_gradient_graph = note_number_to_wave(midi_array[:,:int(88100/length_factor)], end_gradient = True, start_gradient = True, rescale_factor = length_factor)

    wav_chroma = librosa.feature.chroma_stft(y=orig_wav[:88100], sr=22050, tuning=0, norm=2,
                                             hop_length=hop_size, n_fft=n_fft)
    
    midi_chroma = librosa.feature.chroma_stft(y=midi_wave, sr=22050, tuning=0, norm=2,
                                             hop_length=hop_size, n_fft=n_fft)

    D, wp = librosa.sequence.dtw(X=midi_chroma, Y=wav_chroma, metric='cosine')
    wp_s = np.asarray(wp) * hop_size / 22050
    print("cLAPS",D.shape,wp.shape,wp_s.shape,wav_chroma.shape,midi_chroma.shape)

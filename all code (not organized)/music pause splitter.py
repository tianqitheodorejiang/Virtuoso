...
def note_number_to_wave(note_number, gradient_fraction=3, end_gradient = True, start_gradient = False, rescale_factor=1):
                ...
                try:
                    if i+500 < channel.shape[0] and note[1] != channel[i-1][1] and channel[i][1] == channel[i+500][1] and channel[i][1] != channel[i-500][1]: ## every note start
                        ...
                        while True:
                            ...
                            wave_duration += 1
                            ind+=1
                        if channel[i+int(wave_duration/2)][0]<20:
                            print("\n\nBRUUUUUUUUUUUU\n\n")
                        ...
                        for g in range(0,general_gradient_amt):
                            general_gradient.append(g/general_gradient_amt+0.000001)
    ...
    print("ur stoopid:",np.min(ovlp),np.max(ovlp),np.min(np.abs(actual_wave)),np.max(actual_wave))
...
base_output = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/synced/autosyncing wo pauses/"
...
for set_num in range(start_index, len(sets)):
    ...
    while True:
        ...
        midi_wave, midi_graph, start_gradient_graph, end_gradient_graph = note_number_to_wave(midi_array, end_gradient = True, start_gradient = True, rescale_factor = length_factor*(sampling_size/real_length))
        #midi_wave = midi_wave[:sampling_size*2]
        minute = 22050*60
        while True:
            sd.play(midi_wave[minute-44100:minute+44100],22050)
            input()
        actual_midi_wave = midi_wave[:sampling_size]
        wav = orig_wav[wav_tick:wav_tick+sampling_size]

        midi_add = actual_midi_wave.shape[0]
        extra_midi_add = 0
        wav_add = sampling_size
        first = False
        for n,moment in enumerate(midi_wave):
            if moment!=0:
                first = True
            if moment==0 and first:
                print(n)
                '''
                print("\n\nSKIPPING\n\n",(actual_midi_wave[n+1]),n)
                ##Midi end
                midi_empty_duration_plus = 1
                while True:
                    if actual_midi_wave[n+midi_empty_duration_plus] != 0:
                        break
                    midi_empty_duration_plus+=1
                midi_empty_end = n+midi_empty_duration_plus
                ##Midi start
                midi_empty_duration_minus = 1
                while True:
                    if actual_midi_wave[n-midi_empty_duration_minus] != 0:
                        break
                    midi_empty_duration_minus+=1
                midi_empty_start = n-midi_empty_duration_minus+1
                current_array = current_array[:int(midi_empty_start/length_factor)]
                midi_wave = midi_wave[:midi_empty_start]

                midi_add = midi_empty_start
                extra_midi_add = midi_empty_end-midi_empty_start
                start+=1
                break'''
        input()
        first = False
        for n,moment in enumerate(wav):
            if moment!=0:
                first = True
            if np.max(np.abs(wav[n:n+100]))==0 and first:
                print("\n\nAYYYAYAYYSDYFASYDFYASYUDFYASDFYUASDYF\n\n")
                ##Wav end
                wav_empty_duration_plus = 1
                while True:
                    if abs(wav[n+wav_empty_duration_plus]) > 0.03:
                        break
                    wav_empty_duration_plus+=1
                wav_empty_end = n+midi_empty_duration_plus
                ##Wav start
                wav_empty_duration_minus = 1
                while True:
                    if abs(wav[n-wav_empty_duration_minus]) > 0.03:
                        break
                    wav_empty_duration_minus+=1
                wav_empty_start = n-wav_empty_duration_minus+1
                
                wav = wav[:wav_empty_start]
                
                wav_add = wav_empty_end
                break
        ...
        save_index+=1
        wav_tick += wav_add
        midi_tick += (midi_add/best)+extra_midi_add

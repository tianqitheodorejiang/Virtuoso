...
for set_num in range(start_index, len(sets)):
    ...
    while True:
        ...
        for good in range(70,71):
            ...
            wav = np.abs(wav)
            #midi_wave = skimage.transform.rescale(midi_wave,((1/880)))
            #wav = skimage.transform.rescale(wav,((1/880)))
            midi_wave /= np.max(midi_wave)
            wav /= np.max(wav)
            #midi_wave = (midi_wave > 0.7).astype("float64")
            #wav = (wav > 0.7).astype("float64")
            ...
            for t,thing in enumerate(abs_midi):
                add = np.zeros((100))
                if abs_midi[t-1]<thing and abs_midi[t+1]<thing:
            ...
            midi_amp = midi_amp[::220]#skimage.transform.rescale(midi_amp,((1/880,1)))
            wav_amp = wav_amp[::220]#skimage.transform.rescale(wav_amp,((1/880,1)))
            ...
            for i in range(70,130):
                test = skimage.transform.rescale(midi_amp,((i/100),1))

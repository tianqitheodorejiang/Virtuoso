...
save_folder_path = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/Midis and Mels for Machine Learning mel2mel 128 nmels 64 time split half frame double stops emphasis with gradient"
...
for set_num in range(0,len(pathes)):
    ...
    if set_num == 1:
        ...
        one,complete_midi_graph,_,_ = note_number_to_wave(midi_graph, end_gradient = True, start_gradient = True)
        print("wav shape:",one.shape)
        ...
        complete_midi_wave = midi_wave*0.1/np.max(midi_wave)
    ...
    ax.imshow(midi_specgram[:1600])
    ax = fig.add_subplot(1, 2, 2)
    ax.imshow(wav_specgram[:1600])
    plt.show()'''
...
new_indexes = []
for i in range(0,len(midis[0])):
    index = random.randint(0,len(midis[0])-1)
...
for i in range(0,4):
    for index in new_indexes:
        print(index)
        new_midis[i].append(midis[i][index])
        new_wavs[i].append(wavs[i][index])
...
print("Loaded in" ,len(midis),len(wavs), "sets from", sets, "folders in", int((time.time() - start_time)/60), "minutes and",
          int(((time.time() - start_time) % 60)+1), "seconds.")
midi_sets, wav_sets = new_midis,new_wavs#seperate_sets(new_midis, new_wavs, set_size)
print(len(midi_sets))

...
def make_wave_classic(duration):
    wave = []
    for i in range(0,duration+1):
        wave.append(i*2*np.pi)
...
def generate_specgrams(output_path, max_pitch):
    ...
    ov = N - Fs // 1000
    #ov = 2025
    for pitch in range(0,max_pitch):
        print(pitch)
        freq = 440*(2**((pitch-69)/12))

        wave = make_wave(freq, 50, sample_rate=22050)


        f,t,specgram = stft(wave,nfft=N,window=w,nperseg=None,noverlap=ov)
        print(specgram.shape)

        for i in range(0,5):
            save_array(specgram[:,i*10000:(i+1)*10000], output_path+"/"+str(pitch)+"-"+str(i+1))
            print(specgram[:,i*10000:(i+1)*10000].shape)
        print("took", int((time.time() - start_time)/60), "minutes and",int(((time.time() - start_time) % 60)+1), "seconds.")
...
save_path = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/frequencies 3"

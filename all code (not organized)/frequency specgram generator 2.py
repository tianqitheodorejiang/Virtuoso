...
def generate_specgrams(output_path, max_pitch):
    ...
    for pitch in range(69,max_pitch):
        ...
        for thing in specgram:
            print(np.max(np.abs(thing)))
        print(np.median(specgram[0]))
        print(np.median(specgram[-1]))
        print("yeet")
        start = np.median(specgram[0])
        less_than = False
        for n,thing in enumerate(specgram):
            if np.median(specgram[-n])<start:
                last = len(specgram)-n
                break
        differences = []
        for n,thing in enumerate(specgram):
            differences.append(start-np.median(specgram[n]))
        differences = np.abs(differences)
        min_ = 1000
        for d,difference in enumerate(differences):
            if specgram[d-1] < specgram[d]:
                if difference < min_:
                    min_ = difference
                    best_match = d
        best_match = list(differences).index(np.min(differences[1:]))
        print(differences[best_match])
        print("best index:",best_match)
        cut_spec = []
        for i in range(0,last):
            cut_spec.append(specgram[i])
        cut_spec.append(specgram[0])
        specgram = np.stack(cut_spec)
        print("cut shape",specgram.shape)
        two = []

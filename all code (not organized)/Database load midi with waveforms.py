...
def midi_2_specgram(midi, freqs, bins=0):
    Fs = 22050
    N = 2048
    w = np.hamming(N)
    ov = N - Fs // 1000
    waves = []
    max_freq = 440*(2**((np.max(midi)-69)/12))
    note_specgrams = {}
    for freq in freqs:
        print(freq)
        if freq==0:
            note_specgrams[str(freq)] = np.zeros((freqs.shape[0],midi.shape[0]))
            print(note_specgrams[str(freq)].shape)
        else:
            wave = make_wave(freq, midi.shape[0]/1000, sample_rate=22050)
            _,_,note_specgrams[str(freq)] = stft(wave,nfft=N,fs=Fs,window=w,nperseg=None,noverlap=ov)
            print(note_specgrams[str(freq)].shape)
        if freq > max_freq:
            break
    ...
    for i,note in enumerate(midi):
        for channel in range(0,4):
            if note[0,channel] != 0:
                ...
                specgram[:][i]+=note_specgrams[str(note_Hz)]
    return specgram
...
for set_ in os.listdir(path):
    ...
    if not found_wav or not found_mid:
        ...
        midi_array = skimage.transform.rescale(np.stack(midi_array), ((wav_specgram.shape[1]/midi_array.shape[0]), 1, 1))
        specgram = midi_2_specgram(midi_array, f)
        t,back = istft(specgram*0.1,nfft=N,fs=Fs,window=w,nperseg=None,noverlap=ov)
        sd.play(back,22050)

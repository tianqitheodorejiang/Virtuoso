...
base_output = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/synced/autosyncing final/"
...
for set_num in range(start_index, len(sets)):
    ...
    while True:
        ...
        current_array = midi_array[:,int(midi_tick/length_factor):int((midi_tick/length_factor)+(real_length/length_factor)*4)+1]
        ...
        midi_wave = midi_wave[:sampling_size*4]
        if midi_wave.shape[0] < sampling_size*5:
            padding_amt = sampling_size*5-midi_wave.shape[0]
        ...
        big_wav = orig_wav[wav_tick+sampling_size:wav_tick+sampling_size*2]
        #sd.play(wav,22050)
        if wav.shape[0] < sampling_size:
            ...
            for pad in padding:
                padded.append(pad)
            wav = np.stack(padded)
        if big_wav.shape[0] < sampling_size:
            padding_amt = sampling_size-big_wav.shape[0]
            padding = np.zeros(padding_amt)
            padded = []
            for time_ in big_wav:
                padded.append(time_)
            for pad in padding:
                padded.append(pad)
            big_wav = np.stack(padded)
        ...
        wav_chroma_dos = librosa.feature.chroma_stft(y=big_wav, sr=22050, tuning=0, norm=2,
                                                 hop_length=hop_size, n_fft=n_fft)
        ...
        ax.imshow(wav_chroma_dos)
        plt.show()'''

        for i in range(50,200):
            test1 = skimage.transform.rescale(midi_chroma,(1, i/100))
            test1 /=(np.max(test1)+0.00000000001)
            test1 = test1[:,:wav_chroma.shape[1]]
            
            test_wav1 = wav_chroma.copy()
            test_wav1/=(np.max(test_wav1)+0.00000000001)
            #print("maxes:",np.max(test),np.max(test_wav))
            err = (test1-test_wav1)**2
            #err[test==0] = 0
            mse1 = np.sum(err)
            for j in range(50,200,10):
                #print(midi_chroma[:,int(/(i/100)):].shape)
                #print(int(sampling_size/(i/100)))
                test = skimage.transform.rescale(midi_chroma[:,int(test1.shape[1]/(i/100)):],(1, j/100))
                test /= (np.max(test)+0.00000000001)
                test = test[:,:wav_chroma_dos.shape[1]]
                
                test_wav = wav_chroma_dos.copy()

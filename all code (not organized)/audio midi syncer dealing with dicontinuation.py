        ...
        for i,note in enumerate(channel):
            if note[0]>0: ## pitch
                try:
                    ...
                    cont = input("...")
    '''print("before ugh")
    for n,thing in enumerate(start_gradients[0]):
        if thing == 0 and end_gradients[0][n-1] == 0:
            print("ugh")'''
...
for set_num in range(start_index, len(sets)):
    ...
    while True:
        ...
        while good != "y":
            ...
            midi_wave  = duration_2_wave(midi_duration, gradients = [end_gradient_graph, start_gradient_graph])

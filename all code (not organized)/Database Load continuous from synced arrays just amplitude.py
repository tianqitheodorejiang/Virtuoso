...
save_folder_path = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/Midis and Mels for Machine Learning continuous end gradient"
...
for set_num in range(0,1):
    ...
    y = load_wave(path+"/"+str(set_num)+"/wavs")
    y = y*0.1/np.max(y)
    ...
    wav_specgram = np.stack(wav_specgram)/np.max(wav_specgram)

    print("wav min max:",np.min(wav_specgram),np.max(wav_specgram))
    ...
    ###loading in midi------------------------------------------------------------
    midi_graph = load_graph(path+"/"+str(set_num)+"/midis/no gradient")
    start_gradient_graph = load_graph(path+"/"+str(set_num)+"/midis/start gradient graphs")
    end_gradient_graph = load_graph(path+"/"+str(set_num)+"/midis/end gradient graphs")
    ...
    midi_specgram = np.stack(midi_specgram)/np.max(midi_specgram)

    print("midi min max:",np.min(midi_specgram),np.max(midi_specgram))
    ...
    print(np.unique(np.round(timef_wav,1),return_counts=True))
    ...
    converted_back_midi[converted_back_midi < 0.0] = 0
    ...
    '''
    converted_back_wav = np.transpose(timef_midi)

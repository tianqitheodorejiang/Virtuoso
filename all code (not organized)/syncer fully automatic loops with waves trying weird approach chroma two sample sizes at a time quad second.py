...
for set_num in range(start_index, len(sets)):
    ...
    while True:
        ...
        orig_midi_wave, midi_graph, start_gradient_graph, end_gradient_graph = note_number_to_wave(current_array, end_gradient = True, start_gradient = True, rescale_factor = length_factor*(sampling_size/real_length))
        #sd.play(midi_wave,22050)
        #input("yayeet")
        print("\n\n\n\n\nMIDI SHAPE:",orig_midi_wave.shape,"\n\n\n\n\n")
        midi_wave = orig_midi_wave[:sampling_size*3]
        if midi_wave.shape[0] < sampling_size*3:
            padding_amt = sampling_size*3-midi_wave.shape[0]
        ...
        wav_chroma_dos = librosa.feature.chroma_stft(y=orig_wav[wav_tick+sampling_size:wav_tick+sampling_size*4], sr=22050, tuning=0, norm=2,
                                                 hop_length=hop_size, n_fft=n_fft)
        
        midi_chroma = librosa.feature.chroma_stft(y=midi_wave, sr=22050, tuning=0, norm=2,
                                                 hop_length=hop_size, n_fft=n_fft)
        midi_chroma_dos = librosa.feature.chroma_stft(y=orig_midi_wave[:sampling_size*5], sr=22050, tuning=0, norm=2,
                                                 hop_length=hop_size, n_fft=n_fft)
        ...
        for i in range(70,130):
            ...
            #err[test==0] = 0
            mse1 = np.sum(err)
            for j in range(70,130,10):
                #print(midi_chroma[:,int(/(i/100)):].shape)
                #print(int(sampling_size/(i/100)))
                test = skimage.transform.rescale(midi_chroma_dos[:,int(test.shape[1]/(i/100)):],(1, j/100))
                test /= (np.max(test)+0.00000000001)

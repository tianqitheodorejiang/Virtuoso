...
class hp:
    ...
    max_T = 32 # Maximum number of mel frames.
...
def duration2notes(duration, wav, max_size, gradients = None, rescale_factor=1):
    midi_wave = []
    last = 0
    lengths = []
    starts = []
    ...
    actual_wave = actual_wave/np.max(actual_wave)
    ...
    both_gradients = gradients[0].copy()
    both_gradients[:] = 1
    for gradient in gradients:
        both_gradients*=gradient
    start_index = 0
    keep_going = True
    while keep_going:
        index = 0
        while True:
            try:
                if starts[start_index+index]-starts[start_index]>max_size:
                    beg = starts[start_index]
                    too_long = False
                    if starts[start_index+index-1]==beg:
                        end = starts[start_index+index-1]
                    else:
                        too_long = True
                        end = beg+max_size
                    ...
                    print(beg,end)
                    segment = np.zeros(end-beg)
                    wav_segment = wav[beg:end]
                    
                    for i in range(0,1800):
                        wav_segment[i]*=(i/1800)
                        wav_segment[-i]*=(i/1800)
                        
                    for n,channel in enumerate(midi_wave[:,beg:end]):
                        gradient = both_gradients[n][beg:end]
                        for i in range(0,1800):
                            gradient[i]=(i/1800)
                            gradient[-i]=(i/1800)
                    ...
                    note_groups.append(segment)
                    wav_note_groups.append(wav_segment)
                ...
                beg = starts[start_index]
                too_long = False
                if len(wav)-1-beg <10000:
                    keep_going = False
                    break
                elif len(wav)-1-beg<=max_size:
                    end = len(wav)-1
                    print("not too long")
                else:
                    print("too long")
                ...
                print(beg,end)
                segment = np.zeros(midi_wave[:,beg:end].shape[1])
                wav_segment = wav[beg:end]
    ...
    return note_groups,wav_note_groups


def note_number2notes(note_number, wav, max_size, gradient_fraction=3, end_gradient = True, start_gradient = True, rescale_factor=1):
    last = 0
    rescaled_note_number = np.round(skimage.transform.rescale(note_number, (1, rescale_factor, 1)))
    midi_wave = rescaled_note_number.copy()[:,:,0]
    start_gradients = rescaled_note_number.copy()[:,:,0]
    ...
    start_gradients[:] = 0
    end_gradients[:] = 0
    note_indexes = rescaled_note_number.copy()[:,:,0]
    note_indexes[:] = 0
    starts = []
    for n,channel in enumerate(rescaled_note_number):
        for i,note in enumerate(channel):
            if note[0]>0: ## pitch
                try:
                    if i+500 < channel.shape[0] and note[1] != channel[i-1][1] and channel[i][1] == channel[i+500][1] : ## every note start
                        wave_duration = 1
                        ind = 0
                        while True:
                            if i+ind >= channel.shape[0]-1 or i+ind+500 >= channel.shape[0] or (note[1] != channel[i+ind+1][1] and channel[i+ind+1][1] == channel[i+ind+500][1]):
                                break
                            wave_duration += 1
                            ind+=1
                        ...
                        start_gradients[n][i:i+len(wave)] = 1
                        end_gradients[n][i:i+len(wave)] = 1
                        if i not in starts:
                            starts.append(i)
    ...
    midi_wave = midi_wave[:][:last+1]
    both_gradients = start_gradients*end_gradients
    actual_wave = np.zeros(midi_wave[0].shape[0])
    for n,channel in enumerate(midi_wave):

        channel*=both_gradients[n]
        actual_wave += channel
...
save_folder_path = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/Midis and Mels for Machine Learning SSRN note starts 32 split"
...
for set_num in range(0,len(pathes)):
    ...
    if set_num == 0:
        ...
        midi_notes,wav_notes = duration2notes(midi_duration,y,hp.max_T*1099, gradients = [start_gradient_graph, end_gradient_graph])
    else:
        midi_notes,wav_notes = note_number2notes(midi_graph,y,hp.max_T*1099)

    print(len(wav_notes))

    print(np.max(wav_notes[0]))

    '''for thing in wav_notes[0]:
        print(thing)'''
    '''for t,thing in enumerate(wav_notes):
        sd.play(thing,22050)
        cont = input()'''
    ...
    '''for t,thing in enumerate(wav_notes):
        sd.play(thing,22050)
        mel, mag = load_spectrograms(thing)
        mel1, mag1 = load_spectrograms(midi_notes[t])
        fig = plt.figure()
    ...
    for mel in mels:
        segment = mel.copy()
        print(mel.shape)
        if mel.shape[0] < time_split:
            print("padding")
            padded = []
            for moment in mel:
                padded.append(moment)

            for i in range(0,time_split-mel.shape[0]):
                padded.append(np.zeros(mel.shape[1]))
            segment = np.stack(padded)
        print(segment.shape)
        if segment.shape[0]>time_split:
            print("\n\nKLJALSDFJLAJSDLKFJLKASJDFLKJALSJFD\n\n")
    ...
    for mag in mags:
        segment = mag.copy()
        if mag.shape[0] < time_split_mag:
            print("padding")
            padded = []
            for moment in mag:
                padded.append(moment)

            for i in range(0,time_split_mag-mag.shape[0]):
                padded.append(np.zeros(mag.shape[1]))
            segment = np.stack(padded)
        print(segment.shape)
        if segment.shape[0]>time_split_mag:
            print("\n\n_________________ASKDJFBKJASHDFKHKASDHF________\n\n")
        segments.append(segment)

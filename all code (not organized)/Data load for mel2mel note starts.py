...
class hp:
    ...
    max_T = 64 # Maximum number of mel frames.
        ...
        while True:
            try:
                if starts[start_index+index]-starts[start_index]>max_size:
                    ...
                    if not too_long:
                        start_index = start_index+(index//2)-1
                    else:
...
save_folder_path = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/Midis and Mels for Machine Learning mel2mel note starts 64 split"
...
for set_num in range(0,len(pathes)):
    ...
    for mag in mags:
        segment = mag.copy()
        if mag.shape[0] < time_split:
            print("padding")
            padded = []
            for moment in mag:
                padded.append(moment)

            for i in range(0,time_split-mag.shape[0]):

...
def duration_2_wave(duration, gradient_fraction = 10, return_different_gradients = False, gradients = None):
    midi_wave = []
    last = 0
    lengths = []
...
def redifine_end_gradient(gradient, factor):
    new_gradients = gradient.copy()
    new_gradients[:] = 1
    for n,channel in enumerate(gradient):
        for i,note in enumerate(channel):
            if note < 1 and channel[i-1] == 1: ##start of a gradient
                duration = 1
                ind = 0
                while True:
                    try:
                ...
                new_gradient = []
                for g in range(0,new_duration):
                    new_gradient.append(g/new_duration)
                for j,grad in enumerate(new_gradient):
                    new_gradients[n,gradient_end-j] = grad
    return new_gradients 
                
                    
def redifine_start_gradient(gradient, factor):
    new_gradients = gradient.copy()
    new_gradients[:] = 1
    for n,channel in enumerate(gradient):
        for i,note in enumerate(channel):
            if note == 0 and channel[i+1] > 0: ##start of a gradient
                duration = 1
                ind = 1
                while True:
                    try:
...
set_size = 48
path = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/synced/waveforms with gradient graphs"
save_folder_path = "C:/Users/JiangQin/Documents/python/Music Composition Project/Music data/violin/Midis and Mels for Machine Learning end gradient binary"
...
for set_num in range(0,1):
    ...
    print(end_gradient_graph.shape)
    #end_gradient_graph = redifine_end_gradient(end_gradient_graph, 0.2)
    #start_gradient_graph = redifine_start_gradient(start_gradient_graph, 3)
    ...
    #timef_midi = np.round(timef_midi,1)
    #timef_midi[timef_midi == 0] = 0.1
    timef_wav = np.transpose(wav_specgram)

    timef_wav = np.round(timef_wav,1)
    timef_wav[timef_wav > 0] = 1
    #timef_wav[timef_midi == 0] = 0
    print(np.unique(timef_wav,return_counts = True))
    timef_wav_onehot = np.stack([timef_wav,timef_wav],axis=-1)
    timef_wav_onehot[:] = 0
    print( timef_wav_onehot.shape)
    timef_wav_onehot[:,:,0][timef_wav == 0] = 1
    timef_wav_onehot[:,:,1][timef_wav == 1] = 1
    #timef_wav = timef_wav_onehot.copy()
    print(np.unique(timef_wav,return_counts=True))
    ...
    converted_back_wav = np.transpose(timef_wav) > 0.5
    converted_back_wav[converted_back_wav < 0.2] = 0
    converted_back_wav[converted_back_wav > 0.7] = 0

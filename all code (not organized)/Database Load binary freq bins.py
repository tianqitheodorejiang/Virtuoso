...
def midi_2_specgram(midi, length_factor):
    ...
    for channel in midi_wave:
        ...
        specgram += np.real(channel_specgram)
    specgram = specgram*0.1/np.max(specgram)
    return specgram
            
def binarize_specgram(specgram, freqs, sample_rate):
    waves = []
    for freq in freqs:
        wave = make_wave(freq, specgram.shape[1], sample_rate)
        waves.append(wave)
        print(freq)
        print(np.max(wave))
        print(np.min(wave))
    new_specgram = []
    for n,bin_ in enumerate(specgram):
        new_bin = bin_/waves[n]
        print(np.min(bin_))
        print(np.max(bin_))
        print(np.min(waves[n]))
        print(np.max(waves[n]))
        print(np.min(new_bin))
        print(np.max(new_bin))
    cont = input("...")
...
for set_num in range(start_index, len(sets_)):
    ...
    for file in os.listdir(os.path.join(path,sets_[set_num])):
        if file.endswith(".wav") and not found_wav:
            ...
            real_specgram = np.real(specgram)
            wav_specgram = []
            abs_specgram = np.abs(specgram)/np.max(np.abs(specgram))


            for i,bin_ in enumerate(real_specgram):
                new_bin = bin_.copy()
                new_bin[abs_specgram[i] < 0.01] = 0
                new_bin[new_bin > 0] = 1
                new_bin[new_bin < 0] = -1
                wav_specgram.append(new_bin)
    ...
    if not found_wav or not found_mid:
        ...
        midi_specgram = midi_2_specgram(midi_array, length_factor)
        print(np.min(midi_specgram))
        print(np.max(midi_specgram))
        binary_midi = midi_specgram.copy()
        binary_midi[midi_specgram > 0.05] = 0.1
        binary_midi[midi_specgram <= 0.05] = 0

        t,back = istft(binary_midi,nfft=N,fs=Fs,window=w,nperseg=None,noverlap=ov)
        sd.play(back,22050)
        cont = input("midi...")
        ...
        midi_specgram += 0.1
        midi_specgram[midi_specgram < 0] = 0
        print(np.max(midi_specgram))
        print(np.min(midi_specgram))
        '''
        ...
        precise_sample_rate = midi_specgram.shape[1]/(wav_length/22050)
        print(precise_sample_rate)
        
        binary_midi = binarize_specgram(midi_specgram, f, precise_sample_rate)
        binary_wav = binarize_specgram(wav_specgram, f, precise_sample_rate)
        ...
        padding = np.zeros((padding_amt, segments[-1].shape[1]))+0.1
        new_last = []
